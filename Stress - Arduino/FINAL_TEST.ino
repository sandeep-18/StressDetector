...
void loop() {
  ...
  if(temp_average > 36 && bpm_average > 70 && O2_average > 45){
    // High Stress
    sendStressInfo(1);
  }

  else if (temp_average > 30 && bpm_average > 60 && O2_average > 30){
    //Medium Stress
    sendStressInfo(2);
  }
  else{
...
void sendStressInfo(int field){
  ...
  if (sendATcommand("AT+CIPSTART=\"TCP\",\"api.thingspeak.com\",80", "OK", 100000)) {
        ...
        if(field == 1){
          ...
          getData = "GET /update?api_key=WLMR1EFG742S9H0P&field4=0.5";
        }
